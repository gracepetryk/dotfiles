...
if (( ${+functions[git-info]} )); then
  zstyle ':zim:git-info:clean' format '%{$fg_bold[green]%}%{󰩖 %G%}'
  zstyle ':zim:git-info:branch' format '%b%{${reset_color}%}'
  ...
  zstyle ':zim:git-info:behind' format '%{${reset_color}%}%{↓%G%}%B'
  zstyle ':zim:git-info:keys' format \
      'status' '%C%S%i%I%u' \
      'prompt' ' on %{$fg[magenta]%} %b%A%B|${(e)git_info[status]:+"${(e)git_info[status]}"}%f%%b'
